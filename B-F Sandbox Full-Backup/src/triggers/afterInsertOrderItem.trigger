trigger afterInsertOrderItem on Order_Item__c (before insert, before update) {
    for (Order_Item__c myOrderItem : Trigger.new) {
        ...
        if (myOrderItem.Price_Ref_Material__c != null)  {
            List<Price_Tolerance__c> GetPA = [Select Max_PA_Per_Case__c, PRM_Units_Per_Case__c From Price_Tolerance__c where Price_Ref_Material__c =:myOrderItem.Price_Ref_Material__c and Banner_Partner__c =:myOrderItem.Banner_Partner__c];
        ...
        if (myOrderItem.Wholesaler__c != null) {
            ...
            if (sapMaterialNumber == null || sapMaterialNumber.length() == 0) {
                ...
                List<Wholesaler_Product__c> GetNum = [Select Name, Description_of_the_Material_Provided__c, id from Wholesaler_Product__c where Wholesaler_Number__c =:myOrderItem.Wholesaler__c and SAP_Material_Number__c =:sapMaterialNumber];
